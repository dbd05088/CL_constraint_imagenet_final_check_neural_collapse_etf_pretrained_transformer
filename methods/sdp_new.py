...
class SDP(CLManagerBase):
    ...
    def add_new_class(self, class_name):
        ...
        self.sdp_model.head = copy.deepcopy(self.model.head)
        self.ema_model_1.head = copy.deepcopy(self.model.head)
        self.ema_model_2.head = copy.deepcopy(self.model.head)
        ...
        if distill:
            if do_cutmix:
                ...
                with torch.cuda.amp.autocast(self.use_amp):
                    ...
                    grad = lam * self.get_grad(logit.detach(), labels_a, self.model.head.weight) + (
                            1 - lam) * self.get_grad(logit.detach(), labels_b, self.model.head.weight)
                ...
                with torch.cuda.amp.autocast(self.use_amp):
                    logit = self.model(x)
                    feature = self.activation["fc_norm"]
                    cls_loss = criterion(logit, y)
                    self.sdp_model.zero_grad()
                    with torch.no_grad():
                        logit2 = self.sdp_model(x)
                        feature2 = self.activation["fc_norm"]
                    distill_loss = ((feature - feature2.detach()) ** 2).sum(dim=1)
                    #self.total_flops += ((feature.shape[0] * feature.shape[1] * 3) / 10e9)

                    sample_weight = self.cls_pred_mean
                    grad = self.get_grad(logit.detach(), y, self.model.head.weight)
                    beta = torch.sqrt((grad.detach() ** 2).sum(dim=1) / (distill_loss.detach() * 4 + 1e-8)).mean()
                    #self.total_flops += (grad.shape[0] * grad.shape[1] * 3 + len(distill_loss)) / 10e9
                    loss = ((1 - sample_weight) * cls_loss + beta * sample_weight * distill_loss).mean()
