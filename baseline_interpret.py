...
for index in range(100, 20000, 100):
...
plt.plot(range(len(between_std)), savgol_filter(between_std, 21, 3), linewidth=3)
...
for key in between_dist_std.keys():
    plt.plot(range(max_len)[-len(between_dist_std[key]):], savgol_filter(between_dist_std[key], 15, 3), label=key)
plt.legend()
